...
export class UpdateSessionDialogComponent {
  ...
  onSelectionChange(selectedValue: string): void {
    ...
    if (selectedValue !== 'other') {
      const newTrackControl = this.sessionForm.get('newTrack');
      if (newTrackControl) {
        newTrackControl.setValue('');
      }
    }
  }
  ...
  async saveChanges(): Promise<void> {
    ...
    if (!this.sessionForm.valid) {
      if (this.dialogData.displayErrorMessageFn) {
    ...
    if (this.dialogData.adjustSessionTimesFn) {
      ...
      // If adjustSessionTimesFn is not provided, save directly
      try {
        const formattedSessionDetails = this.updateSessionTimes([sessionData]);
        await this._backendApiService
          .updateAgenda(formattedSessionDetails)
          .toPromise();
        this.dialogRef.close('SUCCESS');
      } catch (error) {
        if (this.dialogData.displayErrorMessageFn) {
